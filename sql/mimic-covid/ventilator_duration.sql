...
(
    ...
    , MAX(CASE
        ...
        WHEN tidal_volume_spontaneous IS NOT NULL THEN 1
        WHEN tidal_volume_set IS NOT NULL THEN 1
        WHEN plateau_pressure IS NOT NULL THEN 1
        WHEN peep IS NOT NULL THEN 1
        WHEN od.o2_delivery_device_1 = 'Endotracheal tube' THEN 1
        WHEN od.o2_delivery_device_2 = 'Endotracheal tube' THEN 1
        WHEN od.o2_delivery_device_3 = 'Endotracheal tube' THEN 1
        WHEN od.o2_delivery_device_4 = 'Endotracheal tube' THEN 1
  ...
  select
    ...
    , case
        when MechVent = 1 then
          LAG(charttime, 1) OVER (partition by subject_id, MechVent order by charttime)
        else null
...
(
  ...
  , case when MechVent=1 or Extubated = 1 then
      SUM( newvent )
      OVER ( partition by subject_id order by charttime )
    else null end
    as ventnum
...
from vd2
group by subject_id, vd2.ventnum
